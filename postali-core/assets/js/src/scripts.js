...
jQuery( function ( $ ) {
    ...
    $("<div class='all-pages'> + <span></span></div>").insertBefore('.children');
    ...
    // script to make accordions function
	$(".body_accordions").on("click", ".body_accordions_title", function() {
        // will (slide) toggle the related panel.
        $(this).toggleClass("active").next().slideToggle();
        $(this).parent().toggleClass("active");
    });

    // script to make sidebar 'more topics' accordion function
	$(".more-topics").click(function() {
        $('.sidebar-more').slideToggle(400);
    });

    // homepage results box
    $('#results_headline').click(function() {	
        $(this).parent().toggleClass('opened');
    });	
